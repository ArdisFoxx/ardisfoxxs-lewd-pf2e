...
* 8.0.0 - AFLR multi-system release. The full Carnal H-Scene toolkit (arousal, positions, cumflation, kinks, pregnancy, voices, and procedural visuals) now runs natively on Daggerheart alongside Pathfinder 2e, with a Daggerheart-native Carnal action set (Press, Resist, Rescue, Escape) resolving on the duality dice. Cum and Coomer reworked into two small numbers - per-shot Cum set by size, and Coomer loads before a rest - with uncapped big-creature loads that overflow a hole's capacity of 8 and flood the floor out to a size-based range band, mopped into Vials of Cum with the Cum Rag. New transformation content (genitalia and sex-change spells, consumables, and cursed gear, plus adversaries that remake the defeated). New adversaries across every tier topped by the gargantuan Brimstone Brood-Wyrm; new bondage gear, loot, and consumables; new domain cards including the Skyclad and Bimbomancy archetypes. Bestiary stat-aligned by tier and role. Module renamed ArdisFoxx's Lewd Roleplaying (AFLR).
