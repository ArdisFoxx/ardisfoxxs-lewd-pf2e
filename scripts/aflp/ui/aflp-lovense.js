...
window.AFLP_Lovense = {
  ...
  // Per-event vibration patterns using Function command with loopRunningSec/loopPauseSec.
  // Presets (wave, pulse, etc.) are only supported on toys that have them built-in.
  // Loop parameters work on all Lovense toys and give distinct tactile feels per event.
  // Per-event Pattern sequences (apiVer:2 Pattern command).
  // steps: 0-20 normalized shape, scaled at runtime to user's configured min-max range.
  // interval: ms between steps. dur: default timeSec.
  DIRECT_PATTERNS: {
    arousal_low:    { steps: [0,5,10,15,20,15,10,5,0,5,10,15,20,15,10,5],               interval: 500, dur: 8  },
    arousal_medium: { steps: [0,8,16,20,16,8,0,8,16,20,16,8,0,8,16,20],                 interval: 300, dur: 5  },
    arousal_high:   { steps: [4,12,20,12,4,0,4,12,20,12,4,0,20,20,20,4],                interval: 200, dur: 4  },
    cum:            { steps: [20,0,20,0,20,20,20,20,20,20,20,0,20,0,20,20,20,20,20,20],  interval: 100, dur: 8  },
    edge:           { steps: [20,16,12,8,4,0,20,16,12,8,4,0,20,20,20,20,4,0],            interval: 150, dur: 5  },
    horny:          { steps: [6,10,14,18,14,10,6,4,6,10,14,18,14,10,6],                  interval: 500, dur: 8  },
    denied:         { steps: [12,20,12,0,12,20,12,0,12,20,12,0],                         interval: 300, dur: 4  },
    submitting:     { steps: [4,8,12,16,20,16,12,8,4,0],                                 interval: 400, dur: 4  },
    exposed:        { steps: [4,8,4,0,4,8,4,0,4,8],                                      interval: 350, dur: 4  },
    grabbed:        { steps: [20,10,20,10,20,10,20,10],                                   interval: 200, dur: 3  },
    bimbofied:      { steps: [6,10,14,18,14,10,6,10,14,18,14,10],                        interval: 400, dur: 5  },
    mind_break:     { steps: [20,20,20,20,20,20,20,20,20,20,20,20,20,20,20],             interval: 200, dur: 10 },
  },
  ...
  _directFire(eventKey) {
    ...
    // Strength range (0-100 UI scale) → 0-20 Lovense scale
    const minPct = (ev.minStr ?? def.defMinStr ?? 20) / 100;
    const maxPct = (ev.maxStr ?? def.defMaxStr ?? 60) / 100;
    const minL   = Math.round(minPct * 20);
    const maxL   = Math.round(maxPct * 20);

    const pattern = this.DIRECT_PATTERNS[eventKey];
    if (pattern) {
      // Scale normalized 0-20 steps to user's configured min-max range
      const range  = maxL - minL;
      const scaled = pattern.steps.map(v => Math.round(minL + (v / 20) * range)).join(";");
      const minDur = ev.minDur ?? def.defMinDur ?? pattern.dur;
      const maxDur = ev.maxDur ?? def.defMaxDur ?? pattern.dur;
      const dur    = Math.round(minDur + Math.random() * (maxDur - minDur));
      const body   = {
        command:  "Pattern",
        rule:     `V:1;F:v;S:${pattern.interval}#`,
        strength: scaled,
        timeSec:  Math.max(dur, Math.ceil(pattern.steps.length * pattern.interval / 1000) + 1),
        apiVer:   2,
      };
      ...
      }).catch(err => console.warn("AFLP Lovense Direct | connection error:", err.message));
    } else {
      // Fallback: steady vibe for events without a defined pattern
      const strength = Math.round(minL + Math.random() * (maxL - minL));
      const minDur   = ev.minDur ?? def.defMinDur ?? 2;
      const maxDur   = ev.maxDur ?? def.defMaxDur ?? 5;
      const dur      = Math.round(minDur + Math.random() * (maxDur - minDur));
      fetch(this.getDirectUrl(), {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: "Function", action: `Vibrate:${strength}`, timeSec: dur, apiVer: 1 }),
      }).then(r => r.json()).then(data => {
        if (data?.code !== 200) console.warn("AFLP Lovense Direct | unexpected response:", data);
      }).catch(err => console.warn("AFLP Lovense Direct | connection error:", err.message));
    }
...
<div style="font-family:'Helvetica Neue',Arial,sans-serif;color:#ddd;max-width:720px;">
  ...
  _downloadAflpYaml(charName, rows, ch) {
    const L = [
      ...
      "# Place in your GIFT folder: data/events/games/ardisfoxxslewdpf2e/",
      "",
    ];
    ...
    ui.notifications?.info("aflp.yaml downloaded. Place it in your GIFT folder under data/events/games/ardisfoxxslewdpf2e/");
  },
  ...
  _downloadToyMapYaml(charName, rows) {
    const L = [
      ...
      "# Place in the root of your GIFT folder",
      "",
    ];
...
<div>
  ...
  <div id="wiz-gift-new-content" style="display:${isNew ? "block" : "none"};">
    <div style="background:#1a180a;border:1px solid #f0a04a55;border-radius:4px;padding:8px 12px;margin-bottom:8px;">
      <div style="font-size:11px;color:#f0a04a;font-weight:600;margin-bottom:3px;">⚠️ Important: Chrome must run with logging enabled</div>
      <div style="font-size:11px;color:#ccc;line-height:1.6;">GIFT reads Chrome's console log file, and Chrome only writes that file when launched with a logging flag. Make a one-time shortcut: right-click your Chrome shortcut, choose <strong>Create shortcut</strong>, open the new shortcut's <strong>Properties</strong>, and add <code style="background:#111;padding:0 4px;border-radius:3px;color:#c9a96e;">--enable-logging --v=1</code> to the end of the Target field (after the chrome.exe path in quotes). Launch Foundry with that shortcut every session - your normal Chrome shortcut writes nothing, so GIFT would see no events.</div>
    </div>
    <div style="background:#1a1410;border:1px solid #c9a96e22;border-radius:4px;padding:10px 12px;margin-bottom:10px;">
      ...
      <ol style="margin:0;padding-left:18px;font-size:12px;color:#ccc;line-height:2;">
        ...
        <li>
          ...
          <span style="font-size:10px;color:#888;">This file only exists after you've launched Foundry with your logging shortcut at least once. If it's not there, do the launch step below first, then come back and set this.</span>
        </li>
        ...
        <li>Open Foundry using your Chrome logging shortcut (the one with <code style="background:#111;padding:0 3px;border-radius:3px;color:#c9a96e;">--enable-logging</code>, see the note at the top)</li>
  ...
  <div id="wiz-gift-existing-content" style="display:${!isNew ? "block" : "none"};">
    <div style="background:#1a1410;border:1px solid #c9a96e22;border-radius:4px;padding:10px 12px;margin-bottom:10px;font-size:12px;color:#ccc;">
      Download the config files below and drop them into your GIFT folder, replacing the existing ones. Restart GIFT, then open Foundry via your Chrome logging shortcut (Chrome launched with <code style="background:#111;padding:0 3px;border-radius:3px;color:#c9a96e;">--enable-logging --v=1</code>; your normal Chrome shortcut writes no log).
    </div>
  </div>
...
HOW THIS WORKS
--------------
AFLP writes lewd events (arousal, cum, etc.) to the browser console. Chrome only
saves the console to a log file when launched with a logging flag. GIFT watches
that log file and drives your toy. So you must run Foundry in a Chrome that was
started with logging enabled.

STEP 0: CREATE A CHROME LOGGING SHORTCUT (one time)
----------------------------------------------------
1. Right-click your Chrome shortcut and choose "Create shortcut" (or copy it).
2. Right-click the new shortcut and choose Properties.
3. In the Target box, keep the quoted chrome.exe path, add a space, then:
     --enable-logging --v=1
4. (Optional) Rename the shortcut to "Chrome with Logging" so you recognise it.
5. Always launch Foundry with THIS shortcut when you want toy control.
   Your normal Chrome shortcut does not write the log file, so GIFT sees nothing.
...
3. Log File: click "Select Log File". In the file picker's address bar, paste:
   ...
   Note: this file only exists after you've launched Foundry with your logging shortcut at least once.
   If it's not there yet, do STEP 4 first, then return here.
...
STEP 4: TEST IT
-----------------
1. Open Foundry using your Chrome logging shortcut from STEP 0 (NOT your regular Chrome)
2. Open a character sheet, go to the AFLP tab, click the heart button
3. Click "Test Connection". Your toy should buzz.

TROUBLESHOOTING
---------------
- Toy not responding: make sure you opened Foundry with your Chrome logging shortcut (--enable-logging)
