...
<div style="${s.wrap}">
  ...
  <div style="${s.group}">
    ...
    <ul style="${s.ul}">
      ...
      ${only(["dh"], `<li><strong>This is the Daggerheart beta.</strong> Built from the ground up as a Daggerheart expansion rather than a port of the Pathfinder module.</li>`)}
      ...
      ${only(["dh"], `<li><strong>Start in Session Zero.</strong> The <strong>Lewd Levels</strong> dial goes from 1 to 4, setting how much of the module is live at your table, from background flavour to full mid-combat mechanics.</li>`)}
      ...
      ${only(["dh"], `<li><strong>Arousal is the core track.</strong> Carnal actions mark it, and filling it results in climax. <strong>Carnal Press</strong> is a sexual advance against another creature, resisted with a reaction roll against the presser's Difficulty; <strong>Carnal Escape</strong> is an action roll to break free from the press.</li>`)}
      ${only(["dh"], `<li><strong>The climax rule.</strong> Climax while <strong>Submitting</strong> and you mark <strong>Defeat</strong>. Any other climax makes you <strong>Horny</strong> instead, to a maximum of 3 - Horny clears on a rest.</li>`)}
      ...
      ${only(["dh"], `<li><strong>129 lewd domain cards</strong> across every domain, including Dread and Blood from Hope and Fear / the Void.</li>`)}
      ${only(["dh"], `<li><strong>Living bondage.</strong> Restraints grown from mimic-stuff, some disguised as furniture. The GM spends a Fear to have a piece take hold of someone, only another creature can get it off them, and an action roll to remove it that fails with Fear reshapes the living bondage into something even more punishing.</li>`)}
      ${only(["dh"], `<li><strong>The milk economy.</strong> Rest in a milkmaid harness and it draws off one Bottled Milk; a milking station gives two, or three if your tits are Hyper anatomy type. A milking station also catches cum from creatures finishing into you, bottling it rather than leaving it on the floor.</li>`)}
